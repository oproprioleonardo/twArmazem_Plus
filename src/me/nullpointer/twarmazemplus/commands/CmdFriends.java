...
public class CmdFriends extends BukkitCommand {
    ...
    public void openFriendsInventory(Player p, Armazem armazem, int page){
        ...
        while (count < slots.size()) {
            if (subListSpawners.get(count) != null) {
                final OfflinePlayer player = Bukkit.getOfflinePlayer(subListSpawners.get(count));
                inventoryBuilder.setItem(slots.get(count), new Item(Material.getMaterial(397), 1, (short)3).lore("§fEsse jogador está " + (player.isOnline() ? "§aOnline" : "§fOffline") + "&f.", "&cClique para remover da lista de amigos.").onClick(inventoryClickEvent -> {
                    p.closeInventory();
